(function() {
    ...
    function post(url, data, node, callback, head, tail) {
    ...
    function refreshTabs(node, lexemes, head, tail) {
    ...
    function tokenize(node, response, head, tail) {
        var i, j, k, max_i, max_j, max_k, lemma, sentence, replacement, span, ruby, rt;
                    ...
                    function () {
                        ...
                        for (j = 0; j < event.target.childNodes.length; j++) {
                            if (event.target.childNodes[j].nodeType === Node.TEXT_NODE) {
                                maybeTokenize(event.target.childNodes[j]);
                            }
                        }
                    },
                    TIMEOUT);
